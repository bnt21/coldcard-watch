...
is yours, open a [dispute](../../issues/new?template=disputed-address.yml). No proof of
identity is needed, and nothing secret should ever be posted: the address and the
transaction are enough. It gets checked against the chain, and a listing that does not
hold up comes off the site with the totals moved to match. Disputes are looked at before
any other issue.

The case most likely to be wrong is the one the detector cannot see: an owner who followed
Coinkite's advisory, swept their coins to a new wallet at an urgent fee, and produced the
same shape a thief does. If that is what happened, saying so is enough to get it reviewed.
