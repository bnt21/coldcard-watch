...
- **`x-watch.py`** — the scanner. Cron every 30 min on the box. Searches the tracked
...
- **`publish.py`** — the only writer. Verifies an address, edits every coupled site
...
- **`watch-blocks.py`** — the older chain-side scanner (unchanged). Finds clusters
  from the chain outward; `x-watch.py` finds them from the victim side.

## The two verification tiers

- **proven** — the drained funds land in a known attacker address, directly or
...
- **pattern** — the sweep matches the drain fingerprint (1-in/1-out, no change, a
...
- **collector** — the address receives batched single-input sweeps, so it looks like
...
The cc-connect Dobby (Telegram bot) can run these for Brady — "approve `<addr>`" or
...
Refuses anything the chain does not support. A pattern-only address needs
...
- Env: `~/.coldcard-x-env` (600) — `X_BEARER_TOKEN`, `TELEGRAM_BOT_TOKEN`,
